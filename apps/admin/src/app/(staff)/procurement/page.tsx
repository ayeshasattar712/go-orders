...
import { useQuotesByRfq, useRfqRequests } from '@/services/queries';
import { formatCurrency } from '@/lib/utils';

export const metadata: Metadata = { title: 'Procurement' };

export default function ProcurementPage() {
  const rfqs = useRfqRequests();
  const quotesByRfq = useQuotesByRfq();

  const currentRfqs = rfqs;
  const totalValue = currentRfqs.reduce((sum, r) => sum + r.estimatedValue, 0);
  const exampleRfq = currentRfqs.find((r) => r.id === 'rfq_1');
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          value={String(currentRfqs.length)}
          icon={FileText}
          iconTone="primary"
        />
        <KpiCard
      ...
      <Card>
        ...
        <CardContent>
          <ProcurementBoard requests={currentRfqs} />
        </CardContent>
      </Card>
