...
export default function CustomerChatPage() {
  ...
  async function handleStartNewThread() {
    const created = await chatService.createThread({
      ...
      message: 'Hi GoOrder Admin, I have a question about my order.',
    });
    setThreads((current) => [created.thread, ...current.filter((t) => t.id !== created.thread.id)]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Message GoOrder admin about orders, invoices, delivery, or payments — like Daraz support
          chat.
        </p>
      </div>
        ...
        <div className="grid md:grid-cols-[320px_1fr]" style={{ minHeight: '32rem' }}>
          <div className={`border-r ${showConversationOnMobile ? 'hidden md:block' : ''}`}>
            <div className="flex items-center justify-between border-b p-3">
              ...
              <Button size="sm" variant="ghost" onClick={() => void handleStartNewThread()}>
          ...
          <div className={`flex flex-col ${showConversationOnMobile ? '' : 'hidden md:flex'}`}>
            {activeThread ? (
              ...
              <EmptyState
                ...
                description="Start a chat with GoOrder admin about your order or invoice."
                action={
                  <Button onClick={() => void handleStartNewThread()}>
