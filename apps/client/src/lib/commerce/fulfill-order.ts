...
function paymentMethodLabel(method: CheckoutPaymentMethod) {
  if (method === 'BANK_ACCOUNT') return 'Bank transfer';
...
export async function placeMarketplaceOrder(input: PlaceOrderInput) {
        ...
        data: {
            ...
            create: [
              {
                ...
                label: 'Order placed',
                timestamp: now,
                description: `Paid via ${paymentMethodLabel(input.paymentMethod).toLowerCase()} · ${paymentReference}.`,
              },
              {
                status: 'PROCESSING',
          ...
          payment: {
            create: {
              ...
              bankName: isCheque ? 'Cheque' : GOORDER_BANK.bankName,
              accountTitle: GOORDER_BANK.accountTitle,
              accountNumber: isCheque ? null : GOORDER_BANK.accountNumber,
  ...
  await Promise.all([
    notifyCustomer({
      ...
      message: `${paymentMethodLabel(input.paymentMethod)} ${paymentReference} confirmed for ${orderNumber}.`,
