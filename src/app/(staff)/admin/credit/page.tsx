...
export default function AdminCreditPage() {
  ...
  const adjustCreditLimit = useAdminStore((state) => state.adjustCreditLimit);
  const setCreditFrozen = useAdminStore((state) => state.setCreditFrozen);
              ...
              <div key={client.id} className="rounded-xl border p-4">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      adjustCreditLimit(client.id, drafts[client.id] ?? client.creditLimit)
                    }
                  >
                    Update limit
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => adjustCreditLimit(client.id, client.creditLimit + 5000)}
                  >
                    +$5,000
                  </Button>
                  <Button
                    ...
                    variant="outline"
                    onClick={() =>
                      adjustCreditLimit(client.id, Math.max(0, client.creditLimit - 5000))
                    }
                  >
                    -$5,000
                  </Button>
                  {client.creditFrozen ? (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => setCreditFrozen(client.id, false)}
                    >
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => setCreditFrozen(client.id, true)}
                    >
